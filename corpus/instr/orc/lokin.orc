/*
after bretagne
same as lokin but with acc/dec
*/
...
gklokin_div		init 6
gklokin_ratio	init 1.05 ; >1 = decelerating, <1 = accelerating

	instr lokin
Sinstr init "lokin_instr"
	$CORDELIA_QUALITIEs

isegment 		i gkBEATs
idiv				i gklokin_div
iratio_grow		i gklokin_ratio 
...
ionset_last		init -1
while ionset < idur && indx < $lokin_max && isegment > 0 do
	if abs(ionset - ionset_last) > .005 then
		schedule Sinstr, ionset+random:i(0, .005), isegment, idyn/(indx+1), ienv, icps, ich, Sinstr_out
		ionset_last = ionset
	endif
	ionset += isegment_cur
